...
def load_preprocessed_data():
    """Run preprocessing and load the processed data"""
    ...
    y_train = train_processed['Transport_Cost_Log'].values
...
def main():
    ...
    np.save('oof_cb_preds.npy', y_val_pred)
    np.save('oof_cb_true.npy', y_val)
    print("\n[ENSEMBLE] Saved OOF predictions: oof_cb_preds.npy, oof_cb_true.npy")
    ...
    # Save the final model for ensemble
    import joblib
    joblib.dump(final_model, 'catboost_model.pkl')
    print("\n[ENSEMBLE] Saved final model: catboost_model.pkl")
    ...
    np.save('log_preds_cb.npy', test_predictions_log)
    print("[ENSEMBLE] Saved log predictions: log_preds_cb.npy")
