...
# Use the original Transport_Cost as the target
...
# Just like with Linear Regression, one-hot encoding high-cardinality
# features will create too many columns and cause the tree to overfit.
...
# This pipeline bundles preprocessing and the model together
...
# We are tuning the 'regressor' step of the pipeline
# 'regressor__max_depth': How deep the tree can go
# 'regressor__min_samples_leaf': Minimum samples required at a leaf node
param_grid = {
...
# scoring='neg_root_mean_squared_error': Find the model with the best RMSE
...
# Fit the grid search on the *entire* training set
...
# Our final, best model is stored in grid_search.best_estimator_
...
try:
    ...
    # Get importances from the regressor step
...
# A full tree is unreadable. Let's just plot the first 3 levels.
...
# Use the best_estimator_ from the grid search
preds_test = grid_search.predict(X_test)

# --- NO NEED to reverse a log transform ---

# Ensure no negative predictions (unlikely but good practice)
preds_test[preds_test < 0] = 0

# --- Create Submission File ---
# Create submission with Hospital_Id from test data and predicted Transport_Cost
